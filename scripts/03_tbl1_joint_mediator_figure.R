...
table2 <- dat |>
  select(PROTSEG, starts_with("M"), starts_with("Group_")) |>
  tbl_summary(
    statistic = list(all_continuous() ~ "{median} ({p25}, {p75})",
                     all_categorical() ~ "{n} ({p}%)"),
    type = list(M1 ~ "continuous",
                M2 ~ "continuous"),
    missing = "ifany",
    by = PROTSEG
  )
...
joint_mediators_sum <- ggplot(dat, aes(x = M1_sum, y = M2_sum)) +
  ...
  labs(
    x = "Sum of Clonidine Days 1-3 (mg)",
    y = "Sum of Benzos Days 1-3 (mg)"
  ) +
  theme_minimal()

joint_mediators_mean <- ggplot(dat, aes(x = M1_mean, y = M2_mean)) +
  geom_jitter(width = 0.5, height = 0, alpha = 0.7) +
  labs(
    x = "Mean Dose of Clonidine Days 1-3 (mg)",
    y = "Mean Dose of Benzodiazepines Days 1-3 (mg)"
  ) +
  theme_minimal()
...
joint_mediators_median <- ggplot(dat, aes(x = M1_median, y = M2_median)) +
  geom_jitter(width = 0.5, height = 0, alpha = 0.7) +
  labs(
    x = "Median of Clonidine Days 1-3 (mg)",
    y = "Median Benzos Days 1-3 (mg)"
  ) +
  theme_minimal()
