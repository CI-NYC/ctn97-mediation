...
dat <- readRDS(here::here("data/analysis_data/analysis_data_alt_shift.rds")) |>
  as.data.frame() |>
  mutate(adj_1 = case_when(adj_1 == 1 & max_cows_eligible_1 == 1 & max_cows_1 >= 5 & max_cows_missing_indicator_1 == 0 ~ 1,
                           TRUE ~ 0),
         adj_2 = case_when(is.na(adj_2) ~ NA,
                           adj_2 == 1 & max_cows_eligible_2 == 1 & max_cows_2 >= 5 & max_cows_missing_indicator_2 == 0 ~ 1,
                           TRUE ~ 0),
         adj_3 = case_when(is.na(adj_3) ~ NA,
                           adj_3 == 1 & max_cows_eligible_3 == 1 & max_cows_3 >= 5 & max_cows_missing_indicator_3 == 0 ~ 1,
                           TRUE ~ 0),
         adj_4 = case_when(is.na(adj_4) ~ NA,
                           adj_4 == 1 & max_cows_eligible_4 == 1 & max_cows_4 >= 5 & max_cows_missing_indicator_4 == 0  ~ 1,
                           TRUE ~ 0),
         adj_5 = case_when(is.na(adj_5) ~ NA,
                           adj_5 == 1 & max_cows_eligible_5 == 1 & max_cows_5 >= 5 & max_cows_missing_indicator_5 == 0 ~ 1,
                           TRUE ~ 0)
  )
...
L <- list(c("max_cows_missing_indicator_1"), #benzo
          c("max_cows_missing_indicator_2"), #benzo
          c("max_cows_missing_indicator_3"), #benzo
          c("max_cows_missing_indicator_4"), #benzo
          c("max_cows_missing_indicator_5") #benzo
)
...
for (x in c(1))
{
  for (y in c(0))
  {
    ...
    for (i in 14:14) 
    {
      ...
      saveRDS(res, here::here(paste0("results_061725/mediation_", x, "_", y, "_", i, "cows5.rds")))
    }
  }
}